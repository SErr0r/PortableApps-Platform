...
void CPlaylistWindow_Create()
{
	...
	IF_OpenWindow(windows.m_hifPlaylist, "ChillPlayer Playlist", &options.playlist_window_pos, CPC_INTERFACE_STYLE_RESIZING);
...
void CPlaylistWindow_CreateIPEdit(const int iItem, const int iSubItem)
{
	...
	{
		MessageBox(windows.m_hWndPlaylist, "This file's ID3 tag cannot be updated.  This is because ChillPlayer cannot write to this file.", "Cannot update tag", MB_OK | MB_ICONSTOP);
		return;
	}
