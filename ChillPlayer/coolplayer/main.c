...
void    cmdline_usage(HWND hWndCoolPlayer)
{
	...
	MessageBox(NULL, (LPCTSTR) LockResource(globaldata),
			   "ChillPlayer command line options", 0); // text to set
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	{
		MessageBox(GetDesktopWindow(), "No audio devices in this system", "ChillPlayer", MB_ICONSTOP | MB_OK);
		return -1;
	}
	
	options_read();
	
	hMutexOneInstance = CreateMutex(NULL, FALSE, "CHILLPLAYER-045FA840-B10D-2G3-3436-006067709674");
		...
		if (RegisterClass(&wc))
		{
			...
			hWnd =
				CreateWindowEx(WS_EX_ACCEPTFILES | WS_EX_TOOLWINDOW,
							   CLC_COOLPLAYER_WINDOWCLASSNAME, "ChillPlayer",
